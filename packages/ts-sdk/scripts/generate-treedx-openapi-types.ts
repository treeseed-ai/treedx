...
function readContractSource() {
  ...
  for (const [apiPath, methods] of Object.entries(openapi.paths ?? {})) {
    ...
    for (const [method, definition] of Object.entries(methods)) {
      operations.push({ method: method.toUpperCase(), operationId: String(definition?.operationId ?? ""), path: apiPath });
    }
  }
...
export function renderOpenApiTypes() {
  ...
  const operationLiterals = operations
    .map((operation) => `  { method: "${operation.method}", operationId: "${operation.operationId}", path: "${operation.path}" }`)
    .join(",\n");
...
export function renderOpenApiContractArtifacts() {
  ...
  const inventory = operations.map(({ method, operationId, path: apiPath }) => ({ method, operationId, path: apiPath }));
  const contract = {
