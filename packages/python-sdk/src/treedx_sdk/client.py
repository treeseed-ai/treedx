...
from .transport import HttpxTransport, Transport, TreeDxRequest
from .generated import TREEDX_OPENAPI_OPERATIONS
...
class TreeDxClient:
    def __init__(
        ...
        self.config = TreeDxClientConfig(
            base_url=base_url,
            token=token,
    ...
    def version(self) -> Any:
        return self.transport.request(TreeDxRequest(method="GET", path="/api/v1/version")).data

    def whoami(self) -> Any:
        return self.transport.request(TreeDxRequest(method="GET", path="/api/v1/auth/whoami")).data

    def effective_scope(self) -> Any:
        return self.transport.request(TreeDxRequest(method="GET", path="/api/v1/policy/effective-scope")).data

    def auth_mode(self) -> Any:
        return self.transport.request(TreeDxRequest(method="GET", path="/api/v1/auth/mode")).data

    def create_dev_token(self, body: Any | None = None) -> Any:
        return self.transport.request(TreeDxRequest(method="POST", path="/api/v1/auth/dev-token", body=body)).data
    ...
    def operation(
        ...
        if not any(operation["method"] == method and operation["path"] == path for operation in TREEDX_OPENAPI_OPERATIONS):
            raise ValueError(f"Unknown TreeDX OpenAPI operation: {method} {path}")
        resolved = path
        ...
        return self.transport.request(TreeDxRequest(method=method, path=resolved, query=query, body=body, binary_body=binary_body, headers=headers)).data


class TreeDxRegistryClient:
    def __init__(self, client: TreeDxClient) -> None:
        self.client = client
