...
use crate::types::{RecursiveTreeEntry, TreeEntrySummary};
use std::path::Path;
...
pub fn list_tree_recursive(
    ...
    ref_name: &str,
    tree_path: Option<&str>,
) -> Result<Vec<RecursiveTreeEntry>, GitError> {
    let repo = gix::open(repo_path).map_err(|err| GitError::Git(err.to_string()))?;
    let resolved = resolve_ref(repo_path, ref_name)?;
    let object_id = gix::ObjectId::from_hex(resolved.target.as_bytes())
        .map_err(|err| GitError::Git(err.to_string()))?;
    ...
    let mut entries = Vec::new();
    collect_recursive(&tree, path, &mut entries)?;
    entries.sort_by(|a, b| a.path.cmp(&b.path));
    Ok(entries)
}
...
fn collect_recursive(
    tree: &gix::Tree<'_>,
    prefix: &str,
    entries: &mut Vec<RecursiveTreeEntry>,
) -> Result<(), GitError> {
    for entry in tree.iter() {
        let entry = entry.map_err(|err| GitError::Git(err.to_string()))?;
        let name = entry.inner.filename.to_string();
        ...
        let kind = format!("{:?}", entry.inner.mode.kind()).to_lowercase();
        ...
        entries.push(RecursiveTreeEntry {
        ...
        if kind == "tree" {
            let subtree = entry
                ...
                .try_into_tree()
                .map_err(|err| GitError::Git(err.to_string()))?;
            collect_recursive(&subtree, &full_path, entries)?;
        }
    }
    Ok(())
}
