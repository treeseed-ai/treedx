defmodule TreeDbWeb.OpenApiContractTest do
  use TreeDbWeb.ConnCase, async: false

  alias TreeDbWeb.OpenApiContractAssertions
  ...
  test "validates repository and workspace success envelopes", %{openapi: openapi, token: token} do
    repo_path = Path.join(TreeDb.Store.data_dir(), "repos/bare/openapi-contract")
    File.rm_rf!(repo_path)
    File.mkdir_p!(repo_path)
    git(repo_path, ["init", "-b", "main"])
    git(repo_path, ["config", "user.name", "TreeDB Test"])
