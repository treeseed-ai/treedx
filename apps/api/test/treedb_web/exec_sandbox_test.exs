defmodule TreeDbWeb.ExecSandboxTest do
  ...
  test "direct_dev is rejected in connected mode" do
    previous_mode = System.get_env("TREEDB_AUTH_MODE")
    previous_backend = System.get_env("TREEDB_EXEC_BACKEND")
    previous_allow = System.get_env("TREEDB_ALLOW_DIRECT_EXEC_IN_PROD")
    System.put_env("TREEDB_AUTH_MODE", "connected")
    System.put_env("TREEDB_EXEC_BACKEND", "direct_dev")
    System.delete_env("TREEDB_ALLOW_DIRECT_EXEC_IN_PROD")

    try do
      assert {:error, %{code: "sandbox_policy_denied"}} =
               TreeDb.Exec.Backend.run("true", System.tmp_dir!(), 1000, 1000, %{})
    after
      restore_env("TREEDB_AUTH_MODE", previous_mode)
      restore_env("TREEDB_EXEC_BACKEND", previous_backend)
      restore_env("TREEDB_ALLOW_DIRECT_EXEC_IN_PROD", previous_allow)
    end
  end

  test "container sandbox command builder disables network and applies resource limits" do
    {args, sandbox} =
      TreeDb.Exec.Backends.ContainerSandbox.docker_args(
        "/tmp/workspace",
        "/tmp/run",
        "/tmp/run/command.sh",
        5_000,
  ...
  test "container sandbox reports unavailable when docker cannot be found" do
    ...
    System.put_env("TREEDB_EXEC_BACKEND", "container_sandbox")
    System.put_env("PATH", "")

    try do
      assert {:error, %{code: "sandbox_unavailable"}} =
               TreeDb.Exec.Runner.run("true", System.tmp_dir!(), 1000, 1000, %{})
    after
      restore_env("TREEDB_EXEC_BACKEND", previous_backend)
