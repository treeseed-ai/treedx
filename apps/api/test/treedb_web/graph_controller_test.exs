defmodule TreeDbWeb.GraphControllerTest do
  ...
  setup %{conn: conn} do
    ...
    repo_path = Path.join(TreeDb.Store.data_dir(), "repos/bare/graph-api")
  ...
  test "refreshes, searches, reads, queries, relates, and builds subgraphs", %{
    ...
    assert refresh["snapshotRoot"] =~ "treedb://graph/#{repo_id}/"
    assert refresh["metrics"]["totalFiles"] == 2
    assert "docs/readme.md" in refresh["changed"]["added"]
    assert File.dir?(Path.join(TreeDb.Store.data_dir(), "graph/repos/#{repo_id}"))
    ...
    assert query["providerId"] == "treedb-graph-mvp"
