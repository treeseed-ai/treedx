defmodule TreeDxWeb.EndToEndMvpTest do
  ...
  test "authenticated registry-aware repository loop survives restart", %{conn: conn} do
    ...
    {:ok, _} =
      TreeDx.Store.seed_local_records("node_mirror", "http://node-mirror.example.invalid")
    ...
    assert graph_refresh["snapshotRoot"] ==
             "treedx://graph/#{repo_a_id}/#{graph_refresh["graphVersion"]}"
    ...
    assert artifact["uri"] == "treedx://artifact/#{snapshot["snapshotId"]}"
    ...
    assert {:ok, %{"id" => ^repo_a_id}} = TreeDx.Store.get_repository(repo_a_id)

    assert {:ok, %{"repositoryId" => ^repo_a_id}} =
             TreeDx.Store.get_repository_placement(repo_a_id)

    assert {:ok, events} = TreeDx.Store.list_audit_events(%{repoId: repo_a_id, limit: 200})
    assert length(events) > 0

    assert {:ok, %{"graphVersion" => _}} =
             TreeDx.Graph.Native.read_latest_graph_manifest(repo_a_id, branch_name)

    assert {:ok, %{"snapshotId" => snapshot_id}} =
             TreeDx.Store.get_snapshot_manifest(snapshot["snapshotId"])
