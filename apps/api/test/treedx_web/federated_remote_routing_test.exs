defmodule TreeDxFederationRemoteFake do
  def execute(operation, allowed, params, auth_header) do
    send(Application.fetch_env!(:treedx, :federation_test_pid), {
      :remote_execute,
      operation,
      allowed,
      params,
    ...
    case Application.fetch_env!(:treedx, :federation_test_result) do
...
defmodule TreeDxWeb.FederatedRemoteRoutingTest do
  use TreeDxWeb.ConnCase, async: false

  setup do
    Application.put_env(:treedx, :federation_remote_node, TreeDxFederationRemoteFake)
    Application.put_env(:treedx, :federation_test_pid, self())

    on_exit(fn ->
      Application.delete_env(:treedx, :federation_remote_node)
      Application.delete_env(:treedx, :federation_test_pid)
      Application.delete_env(:treedx, :federation_test_result)
    end)
  end
  ...
  test "remote HTTP execution receives reduced path scope and forwarded auth", %{conn: conn} do
    ...
    Application.put_env(:treedx, :federation_test_result, {
      :ok,
      %{
        "results" => [
  ...
  defp remote_repo!(token, node_id) do
    {:ok, _} = TreeDx.Store.seed_dev_records("node_remote", "http://node-remote.example.test")
    path = Path.join(TreeDx.Store.data_dir(), "repos/bare/federated-remote-#{node_id}")
