defmodule TreeDxWeb.ExecControllerTest do
  ...
  setup %{conn: conn} do
    ...
    path = Path.join(TreeDx.Store.data_dir(), "repos/bare/exec-api")
  ...
  test "actor without live exec capability is rejected", %{repo_id: repo_id} do
    ...
    {:ok, repo} = TreeDx.Store.get_repository(repo_id)
    {:ok, resolved} = TreeDx.Git.resolve_ref(repo["localPath"], "refs/heads/main")
