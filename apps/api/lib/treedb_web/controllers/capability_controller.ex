defmodule TreeDbWeb.CapabilityController do
  ...
  def capabilities(conn, _params) do
    with {:ok, principal} <- require_principal(conn),
      ...
      ok(conn, %{capabilities: TreeDb.Capabilities.canonical()})
    else
  ...
  def grants(conn, params) do
    with {:ok, principal} <- require_principal(conn),
         {:ok, _scope} <-
           TreeDb.Capabilities.require_capability(principal, "policy:read", params["repoId"]),
         {:ok, grants} <- TreeDb.Capabilities.list_grants(params) do
  ...
  def put_grant(conn, params) do
    with {:ok, principal} <- require_principal(conn),
         {:ok, _scope} <-
           TreeDb.Capabilities.require_capability(principal, "policy:write", first_repo(params)),
         {:ok, grant} <- TreeDb.Capabilities.put_grant(params) do
      TreeDb.Audit.append("policy.grant.updated", %{
