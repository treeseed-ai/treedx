defmodule TreeDxWeb.ContextController do
  ...
  def build(conn, %{"repo_id" => repo_id} = params),
    do:
      maybe_proxy_repo_read(
        ...
        fn conn ->
          with_principal(conn, &TreeDx.Graph.build_context(repo_id, params, &1))
        end
      )
  ...
  def parse_ctx(conn, %{"repo_id" => repo_id} = params),
    do:
      maybe_proxy_repo_read(
        ...
        [pool: :graph, allow_mirrors?: false],
        fn conn ->
          with_principal(conn, &TreeDx.Graph.parse_ctx(repo_id, params, &1))
        end
      )
