defmodule TreeDxWeb.InternalFederationController do
  use Phoenix.Controller, formats: [:json]
  import TreeDxWeb.ControllerHelpers

  def proxy(conn, params) do
    with {:ok, node_payload} <- TreeDx.Federation.NodeAuth.verify_conn(conn, "proxy"),
      ...
      json(conn, %{
        ...
        bodyEncoding: "base64",
        servedByNodeId: TreeDx.Federation.NodeIdentity.node_id()
      })
    else
      {:cached, response} ->
        json(conn, response)
  ...
  def mirror_export(conn, %{"repo_id" => repo_id}) do
    with {:ok, _node_payload} <- TreeDx.Federation.NodeAuth.verify_conn(conn, "mirror_export"),
         {:ok, export} <- TreeDx.Federation.MirrorTransfer.export(repo_id) do
      ok(conn, export)
    else
  ...
  def mirror_import(conn, %{"repo_id" => repo_id} = params) do
    with {:ok, node_payload} <- TreeDx.Federation.NodeAuth.verify_conn(conn, "mirror_import"),
         {:ok, import} <- TreeDx.Federation.MirrorTransfer.import(repo_id, params, node_payload) do
      ok(conn, import)
    else
      {:error, error} -> error(conn, status_for(error[:code] || error["code"]), error)
    end
  ...
  def health(conn, _params) do
    with {:ok, _node_payload} <- TreeDx.Federation.NodeAuth.verify_conn(conn, "health") do
      ok(conn, %{
        nodeId: TreeDx.Federation.NodeIdentity.node_id(),
        federation: %{status: "healthy"},
        runtime: %{
          pressure: to_string(TreeDx.Runtime.Resources.memory_snapshot().pressure),
          memory: TreeDx.Runtime.Resources.memory_snapshot()
        },
        pools: TreeDx.Runtime.Pool.snapshot()
      })
    else
      {:error, error} -> error(conn, status_for(error[:code] || error["code"]), error)
    end
  ...
  defp reject_absolute_path_payload(%{body: body}) do
    ...
    |> inspect()
    |> then(fn encoded ->
      if String.contains?(encoded, ["localPath", TreeDx.Store.data_dir()]) do
        {:error,
  ...
  defp idempotency_preflight(%{method: method} = request)
    ...
    case TreeDx.Store.get_idempotency_record(key) do
  ...
  defp dispatch_local(request, _node_payload) do
    ...
    headers =
      request.headers
      |> Map.put("x-treedx-internal-dispatch", "true")
      |> Map.put("x-treedx-forward-hop", "0")
  ...
  defp sanitize_headers(headers) do
    Enum.flat_map(headers, fn
      {key, value} ->
        ...
        if key in ["authorization", "x-treedx-node-authorization", "set-cookie"] do
          []
        else
