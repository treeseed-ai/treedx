defmodule TreeDx.Exec.Backends.ContainerSandbox do
  @moduledoc false

  def run(command, cwd, timeout_ms, max_output_bytes, opts \\ %{}) do
    with docker when is_binary(docker) <- System.find_executable("docker"),
         {:ok, network} <- TreeDx.Exec.Backend.network(opts) do
      run_dir = Path.join([TreeDx.Store.data_dir(), "tmp", "exec", unique_id()])
  ...
  def docker_args(cwd, run_dir, command_file, timeout_ms, network, opts) do
    limits = TreeDx.Exec.Backend.resource_limits(opts)
    image = System.get_env("TREEDX_EXEC_CONTAINER_IMAGE") || "alpine:3.20"
    ...
    args = [
      ...
      "sh",
      "-lc",
      "timeout #{timeout_seconds(timeout_ms)} sh /treedx-exec/#{Path.basename(command_file)} > /treedx-exec/stdout 2> /treedx-exec/stderr"
    ]
