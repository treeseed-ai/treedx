defmodule TreeDx.Exec.Materializer do
  @moduledoc false
  ...
  def materialize(ctx) do
    ...
    with {:ok, base_entries} <-
         ...
         {:ok, overlays} <- TreeDx.Store.list_workspace_files(ctx.workspace["id"]),
  ...
  defp path_allowed?(workspace, path) do
    TreeDx.Capabilities.require_paths(workspace["effectiveScope"] || %{}, [path]) == :ok
  end
