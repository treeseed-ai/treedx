defmodule TreeDx.Repos do
         ...
         {:ok, repo} <-
           put_repository_record(
             %{
               ...
               storageRelativePath:
                 TreeDx.RepositoryStorage.storage_relative_path(repository_name),
               localPath: "",
  ...
  def register(params, principal) do
    with {:ok, _scope} <- TreeDx.Capabilities.require_capability(principal, "repos:write", nil),
         ...
         {:ok, placement} <- ensure_placement(repo) do
      TreeDx.Audit.append("repo.registered", %{
        actor_id: principal["actorId"],
        tenant_id: principal["tenantId"],
        repo_id: repo["id"],
        data: %{name: repo["name"]}
  ...
  def list(principal) do
    with {:ok, _scope} <- TreeDx.Capabilities.require_capability(principal, "repos:read", nil) do
  ...
  def get(repo_id, principal) do
    with {:ok, _scope} <-
         ...
         {:ok, repo} when is_map(repo) <- TreeDx.Store.get_repository(repo_id) do
      {:ok, public_repo(repo)}
    else
  ...
  def status(repo_id, principal) do
    with {:ok, _scope} <-
         ...
         {:ok, git} <- TreeDx.Git.inspect_repository(TreeDx.RepositoryStorage.path!(repo)),
         {:ok, placement} <- TreeDx.Store.get_repository_placement(repo_id) do
      TreeDx.Audit.append("repo.status_inspected", %{
        actor_id: principal["actorId"],
        tenant_id: principal["tenantId"],
        repo_id: repo_id
      })
  ...
  def refs(repo_id, principal) do
    with {:ok, _scope} <- TreeDx.Capabilities.require_capability(principal, "git:read", repo_id),
         {:ok, repo} when is_map(repo) <- TreeDx.Store.get_repository(repo_id),
         {:ok, refs} <- TreeDx.Git.list_refs(TreeDx.RepositoryStorage.path!(repo)) do
      TreeDx.Audit.append("repo.refs_listed", %{
        actor_id: principal["actorId"],
        tenant_id: principal["tenantId"],
        repo_id: repo_id
      })
  ...
  def remotes(repo_id, principal) do
    with {:ok, _scope} <- TreeDx.Capabilities.require_capability(principal, "git:read", repo_id),
         {:ok, repo} when is_map(repo) <- TreeDx.Store.get_repository(repo_id),
         {:ok, remotes} <- TreeDx.Git.list_remotes(TreeDx.RepositoryStorage.path!(repo)) do
      TreeDx.Audit.append("repo.remotes_listed", %{
        actor_id: principal["actorId"],
        tenant_id: principal["tenantId"],
        repo_id: repo_id
      })
  ...
  def sync(repo_id, params, principal), do: TreeDx.Pushes.fetch(repo_id, params, principal)

  defp normalize_registration(params) do
    with {:ok, repository_name} <- TreeDx.RepositoryStorage.validate_name(params),
         :ok <- validate_no_remote_register(params) do
      storage_relative_path = TreeDx.RepositoryStorage.storage_relative_path(repository_name)
  ...
  defp ensure_registration_path(input, params) do
    path = TreeDx.RepositoryStorage.path!(input)
    ...
    cond do
      ...
      create? ->
        TreeDx.RepositoryStorage.ensure_git_repository!(path)
  ...
  defp ensure_placement(repo) do
    input = %{
      ...
      primaryNodeId: System.get_env("TREEDX_NODE_ID") || "node_local",
      mirrorNodeIds: [],
    ...
    case TreeDx.Store.put_repository_placement(input) do
      {:ok, placement} -> {:ok, placement}
      :ok -> TreeDx.Store.get_repository_placement(repo["id"])
      other -> other
    end
  end

  defp put_repository_record(input, repository_name) do
    case TreeDx.Store.put_repository(input) do
      {:ok, repo} ->
        {:ok, repo}
  ...
  defp validate_legacy_registered_path(local_path) do
    data_dir = Path.expand(TreeDx.Store.data_dir())
    ...
    cond do
      ...
      !String.starts_with?(Path.expand(local_path), data_dir) ->
        {:error, %{code: "validation_error", message: "localPath must be under TREEDX_DATA_DIR."}}
  ...
  defp validate_registered_path(path) do
    with {:ok, git} <- TreeDx.Git.inspect_repository(path) do
      cond do
        git["exists"] != true ->
  ...
  defp import_source_path(path) when is_binary(path) do
    with :ok <- TreeDx.RepositoryStorage.validate_relative_path(path) do
      relative = path |> URI.decode() |> String.split("/", trim: true) |> Enum.join("/")
      source = Path.expand(relative, TreeDx.Store.data_dir())
      data_dir = Path.expand(TreeDx.Store.data_dir())
  ...
  defp copy_import_source(repository_name, source) do
    destination = TreeDx.RepositoryStorage.managed_path(repository_name)
