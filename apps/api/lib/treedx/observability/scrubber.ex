defmodule TreeDx.Observability.Scrubber do
  @moduledoc false
  ...
  @treedx_secret ~r/TREEDX_[A-Z0-9_]*(SECRET|TOKEN|PASSWORD|KEY|CREDENTIAL)[A-Z0-9_]*/
  @tmp_path ~r/\/tmp\/[^\s"]*/i
  @var_path ~r/\/var\/lib\/treedx[^\s"]*/i
  ...
  def secret_like?(value) when is_binary(value) do
    value =~ @bearer or value =~ @jwt or value =~ @credential_url or value =~ @treedx_secret or
  ...
  defp data_dir_path?(value) do
    data_dir = TreeDx.Store.data_dir() |> Path.expand()
