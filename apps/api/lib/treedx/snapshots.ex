defmodule TreeDx.Snapshots do
  ...
  defp do_build(repo_id, params, principal) do
    with {:ok, ctx} <- build_context(repo_id, params, principal),
      ...
      {:error, error} = failure ->
        TreeDx.Audit.append("snapshot.build.failed", %{
  ...
  def get(repo_id, snapshot_id, principal) do
    with {:ok, _scope} <-
           TreeDx.Capabilities.require_any(
             principal,
             ["snapshot:build", "artifact:export"],
             repo_id
           ),
         {:ok, manifest} when is_map(manifest) <- TreeDx.Store.get_snapshot_manifest(snapshot_id),
  ...
  defp do_export(repo_id, params, principal) do
    with {:ok, _scope} <-
           TreeDx.Capabilities.require_capability(principal, "artifact:export", repo_id),
      ...
      TreeDx.Audit.append("artifact.exported", %{
  ...
  def download(repo_id, params, principal) do
    with {:ok, _scope} <-
         ...
         {:ok, %{"contentBase64" => content}} <-
           TreeDx.Store.read_artifact_bytes(manifest["snapshotId"]),
         {:ok, bytes} <- Base.decode64(content || "") do
      TreeDx.Audit.append("artifact.downloaded", %{
        actor_id: principal["actorId"],
        tenant_id: principal["tenantId"],
        repo_id: repo_id,
  ...
  defp build_context(repo_id, params, principal) do
    with {:ok, scope} <-
         ...
         {:ok, resolved_ref} <- TreeDx.Git.resolve_ref(TreeDx.RepositoryStorage.path!(repo), ref),
         {:ok, patterns} <- PathMatch.normalize_patterns(params["paths"] || ["**"]),
         :ok <- TreeDx.Capabilities.require_paths(scope, patterns) do
      {:ok,
       %{
         repo_id: repo_id,
         repo: repo,
         ...
         max_file_bytes: env_int("TREEDX_SNAPSHOT_MAX_FILE_BYTES", @default_max_file_bytes),
         max_total_bytes: env_int("TREEDX_SNAPSHOT_MAX_TOTAL_BYTES", @default_max_total_bytes)
       }}
    else
  ...
  defp persist_snapshot(ctx, files) do
    graph_version =
      if ctx.include_graph do
        case TreeDx.Graph.Native.read_latest_graph_manifest(ctx.repo_id, ctx.ref) do
          {:ok, manifest} when is_map(manifest) -> manifest["graphVersion"]
          _ -> nil
        end
      end

    TreeDx.Store.build_snapshot_artifact(%{
      repoId: ctx.repo_id,
      refName: ctx.ref,
  ...
  defp manifest_for_export(repo_id, %{"snapshotId" => snapshot_id}, _principal)
    ...
    with {:ok, manifest} when is_map(manifest) <- TreeDx.Store.get_snapshot_manifest(snapshot_id),
         :ok <- ensure_repo(manifest, repo_id) do
      {:ok, manifest}
    else
      {:ok, nil} -> {:error, %{code: "snapshot_not_found", message: "Snapshot not found."}}
  ...
  defp manifest_for_export(repo_id, params, principal) do
    with {:ok, %{snapshot: snapshot}} <- build(repo_id, params, principal),
         {:ok, manifest} when is_map(manifest) <-
           TreeDx.Store.get_snapshot_manifest(snapshot["snapshotId"]) do
      {:ok, manifest}
    end
  end
  ...
  defp audit_started(ctx, _params) do
    TreeDx.Audit.append("snapshot.build.started", audit_attrs(ctx, %{}))
    :ok
  end
