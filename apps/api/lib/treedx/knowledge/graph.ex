defmodule TreeDx.Graph do
  @moduledoc false

  alias TreeDx.Graph.{Auth, Builder, ContextPack, Dsl, Filter, Native, RefreshJobs}
  ...
  defp do_refresh(repo_id, params, principal) do
    with {:ok, ctx} <- Auth.context(repo_id, params, principal, "graph:refresh"),
         {:ok, _} <- TreeDx.Capabilities.require_capability(principal, "files:read", repo_id),
         {:ok, _} <- TreeDx.Capabilities.require_capability(principal, "git:read", repo_id),
  ...
  def load_authorized_index(ctx, params) do
    with {:ok, manifest} <- Native.read_latest_graph_manifest(ctx.repo["id"], ctx.ref),
         ...
         {:ok, index} <-
           TreeDx.Graph.IndexCache.get_or_load(ctx.repo["id"], manifest["graphVersion"], fn ->
  ...
  defp refresh_or_reuse(ctx, params, previous, refresh_plan) do
    case TreeDx.Graph.RefreshCoordinator.run(ctx, params, previous, refresh_plan, fn ->
  ...
  defp authorized_changed_paths(ctx, params) do
    ...
    |> list_param()
    |> Enum.filter(fn path ->
      match?(:ok, TreeDx.Capabilities.require_paths(ctx.scope, [path]))
    end)
  end
  ...
  defp graph_incremental_max_changed_paths do
    System.get_env("TREEDX_GRAPH_INCREMENTAL_MAX_CHANGED_PATHS", "500")
    |> String.to_integer()
  rescue
    _ -> 500
  end
  ...
  defp snapshot_root(repo_id, graph_version), do: "treedx://graph/#{repo_id}/#{graph_version}"
