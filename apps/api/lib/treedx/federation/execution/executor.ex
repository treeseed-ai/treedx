defmodule TreeDx.Federation.Executor do
  @moduledoc false

  alias TreeDx.Federation.Merge

  def execute(operation, params, principal, auth_header \\ nil) do
    with {:ok, plan} <- TreeDx.Federation.plan(params, principal, plan_opts(operation, params)),
  ...
  defp execute_local(operation, allowed, params, principal) do
    ...
    result =
      case operation do
        :search -> TreeDx.RepositoryQuery.search(allowed.repoId, local_params, principal)
        :query -> TreeDx.RepositoryQuery.query(allowed.repoId, local_params, principal)
        :context -> TreeDx.Graph.build_context(allowed.repoId, local_params, principal)
        :graph -> TreeDx.Graph.query(allowed.repoId, local_params, principal)
      end
  ...
  defp execute_remote(operation, allowed, params, auth_header) do
    if is_binary(allowed.baseUrl) and allowed.baseUrl != "" do
      remote_module =
        Application.get_env(:treedx, :federation_remote_node, TreeDx.Federation.RemoteNode)
