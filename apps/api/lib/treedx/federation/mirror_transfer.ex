defmodule TreeDx.Federation.MirrorTransfer do
  @moduledoc false

  def export(repo_id) do
    with {:ok, repo} when is_map(repo) <- TreeDx.Store.get_repository(repo_id),
         {:ok, repo_path} <- resolve_repository_path(repo),
         {:ok, bundle_path} <- create_bundle(repo_path),
         {:ok, refs} <- TreeDx.Git.list_refs(repo_path),
      ...
      {:ok,
       %{
         ...
         sourceNodeId: TreeDx.Federation.NodeIdentity.node_id()
       }}
    else
  ...
  defp repository_path_candidates(repo, repo_name) do
    [
      safe_path(fn -> TreeDx.RepositoryStorage.path!(repo) end),
  ...
  defp managed_candidate(repo_name) when is_binary(repo_name) and repo_name != "",
    do: TreeDx.RepositoryStorage.managed_path(repo_name)
  ...
  defp validate_git_path(path) do
    with true <- File.exists?(path),
         {:ok, git} <- TreeDx.Git.inspect_repository(path),
  ...
  defp put_mirror_repository(repo_id, repository_name, params) do
    ...
    with {:ok, repo} <- TreeDx.Store.put_repository(input),
  ...
  def sync_remote(repo_id, target_node_id) do
    with {:ok, peer} when is_map(peer) <- TreeDx.Store.get_federation_peer(target_node_id),
         base_url when is_binary(base_url) and base_url != "" <- peer["baseUrl"],
         {:ok, export} <- export(repo_id),
         {:ok, status, _headers, body} <-
           TreeDx.Federation.HttpClient.post_json(
             target_node_id,
             base_url,
  ...
  defp create_bundle(repo_path) do
    bundle_path =
      Path.join(System.tmp_dir!(), "treedx-mirror-#{System.unique_integer([:positive])}.bundle")
  ...
  defp write_temp_bundle(bytes) do
    path =
      Path.join(System.tmp_dir!(), "treedx-import-#{System.unique_integer([:positive])}.bundle")
  ...
  defp clone_mirror(repository_name, bundle_path) do
    with {:ok, normalized} <- TreeDx.RepositoryStorage.validate_name(repository_name) do
      mirror_path = TreeDx.RepositoryStorage.mirror_path(normalized)
  ...
  defp put_assignment(repo_id, params, node_payload) do
    ...
    target_node_id = TreeDx.Federation.NodeIdentity.node_id()
  ...
  defp update_route(repo_id, repository_name, params, assignment) do
    ...
    TreeDx.Store.put_federation_route(%{
      repositoryId: repo_id,
      repositoryName: repository_name,
