defmodule TreeDx.Federation.NodeLoad do
  @moduledoc false

  alias TreeDx.Observability.Metrics

  @table :treedx_federation_node_load
  ...
  def refresh(node_id) do
    ...
    result =
      with {:ok, peer} when is_map(peer) <- TreeDx.Store.get_federation_peer(node_id),
           ...
           {:ok, status, _headers, body} <-
             TreeDx.Federation.HttpClient.get_json(
               node_id,
               base_url,
    ...
    Metrics.incr("treedx_federation_remote_load_refresh_total", %{node: node_id})

    Metrics.observe("treedx_federation_remote_load_refresh_duration_ms", elapsed(started), %{
      node: node_id
    })

    if match?({:error, _}, result) do
      Metrics.incr("treedx_federation_remote_load_refresh_failures_total", %{node: node_id})
    end
  ...
  def local_pool_load(pool) do
    snapshot = TreeDx.Runtime.Pool.pool_snapshot(pool) || %{}
  ...
  defp ttl_ms do
    case Integer.parse(System.get_env("TREEDX_FEDERATION_REMOTE_LOAD_TTL_MS", "2000")) do
