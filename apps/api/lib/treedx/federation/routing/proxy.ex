defmodule TreeDx.Federation.Proxy do
  @moduledoc false
  ...
  def maybe_proxy_repo(repo_id, conn, body \\ nil, mode \\ :write, opts \\ []) do
    if internal_dispatch?(conn) do
      ...
      with {:ok, route} <-
             TreeDx.Federation.RouteTable.resolve_repo(repo_id, [mode: mode] ++ opts) do
  ...
  def maybe_proxy_workspace(workspace_id, conn, body \\ nil) do
    if internal_dispatch?(conn) do
      ...
      with {:ok, route} <- TreeDx.Federation.RouteTable.resolve_workspace(workspace_id) do
        case route["source"] do
          "local" -> :local
          _ -> forward(conn, route, body)
        end
  ...
  defp do_forward(conn, route, body) do
    ...
    case TreeDx.Federation.HttpClient.post_json(
           target_node_id,
           route["baseUrl"],
      ...
      {:error, error} ->
        TreeDx.Observability.Metrics.incr("treedx_federation_read_spillover_failures_total", %{
  ...
  defp forwarded_headers(conn) do
    ...
    |> Enum.reject(fn {key, _value} ->
      key in [
        ...
        "x-treedx-internal-dispatch"
      ]
    end)
    |> Map.new()
    |> Map.put("x-treedx-original-request-id", conn.assigns[:request_id] || "")
    |> Map.put("x-treedx-forwarded-by", TreeDx.Federation.NodeIdentity.node_id())
    |> Map.put("x-treedx-forward-hop", "#{hop(conn) + 1}")
  end
  ...
  defp idempotency_key(conn, target_node_id) do
    case Plug.Conn.get_req_header(conn, "x-treedx-idempotency-key") do
      [key | _] ->
        key
  ...
  defp internal_dispatch?(conn) do
    Plug.Conn.get_req_header(conn, "x-treedx-internal-dispatch") == ["true"]
  end
  ...
  defp record_route(route, mode) do
    TreeDx.Observability.Metrics.incr("treedx_federation_route_selected_total", %{
      mode: to_string(mode),
    ...
    if mode == :read and route["source"] == "remote" and
      ...
      TreeDx.Observability.Metrics.incr("treedx_federation_read_spillover_total", %{
        target_node: route["servedByNodeId"] || "",
        reason: route["reason"] || "unknown",
  ...
  defp route_headers(route) do
    [
      {"x-treedx-route-source", route["source"] || "unknown"},
      {"x-treedx-served-by-node-id", route["servedByNodeId"] || ""},
      {"x-treedx-route-reason", route["reason"] || "unknown"}
    ]
  end

  defp max_hops,
    do: System.get_env("TREEDX_FEDERATION_WRITE_PROXY_MAX_HOPS", "1") |> String.to_integer()

  defp hop(conn) do
    case Plug.Conn.get_req_header(conn, "x-treedx-forward-hop") do
      [value | _] ->
  ...
  defp write_route_required(repo_id) do
    primary =
      case TreeDx.Federation.RouteTable.resolve(repo_id) do
