defmodule TreeDx.Runtime.Pool do
  @moduledoc false
  use GenServer
  ...
  def handle_call({:run, pool, fun, opts}, from, state) do
    ...
    cond do
      ...
      true ->
        ...
        Metrics.incr("treedx_pool_rejections_total", %{
          pool: to_string(pool),
          reason: "queue_full"
        })
  ...
  def handle_info({:execution_timeout, pool, task_ref}, state) do
    ...
    case Map.pop(info.active, task_ref) do
      ...
      {job, active} ->
        if job.task_pid,
          do: Task.Supervisor.terminate_child(TreeDx.Runtime.Pool.TaskSupervisor, job.task_pid)
  ...
  def handle_info({task_ref, result}, state) when is_reference(task_ref) do
    ...
    case find_active(state, task_ref) do
      ...
      {pool, info, job} ->
        ...
        Metrics.observe("treedx_pool_execution_ms", execution_ms, %{pool: to_string(pool)})
  ...
  defp enqueue(info, job) do
    ...
    |> tap(fn _ -> Metrics.incr("treedx_pool_enqueued_total", %{pool: to_string(job.pool)}) end)
  end
  ...
  defp start_job(info, job) do
    ...
    Metrics.incr("treedx_pool_started_total", %{pool: to_string(job.pool)})
    {info, job}
  end
  ...
  defp busy(pool, reason) do
    {:error,
     %{
       code: "server_busy",
       message: "TreeDX is busy processing repository work. Retry later.",
       details: %{
         pool: to_string(pool),
         reason: reason,
         retryAfterMs: 1_000
  ...
  defp publish_pool(pool, info) do
    ...
    Metrics.put_gauge("treedx_pool_active", map_size(info.active), labels)
    Metrics.put_gauge("treedx_pool_size", info.size, labels)
    Metrics.put_gauge("treedx_pool_active_max", info.active_max, labels)
