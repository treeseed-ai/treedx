defmodule TreeDx.Runtime.Resources do
  @moduledoc false

  def cpu_budget do
    int_env("TREEDX_RUNTIME_CPU_BUDGET", System.schedulers_online())
  end
  ...
  def cache_budget_bytes do
    case memory_budget_bytes() do
      ...
      budget ->
        fraction = float_env("TREEDX_CACHE_MEMORY_FRACTION", 0.25)
        min_free = int_env("TREEDX_CACHE_MIN_FREE_MEMORY_MB", 512) * 1_048_576
  ...
  def worker_pool_size(:workspace_mutation),
    do: int_env("TREEDX_WORKSPACE_WORKER_POOL_SIZE", max(2, cpu_budget() * 2))

  def worker_pool_size(:graph), do: int_env("TREEDX_GRAPH_WORKER_POOL_SIZE", max(1, cpu_budget()))
  ...
  def worker_pool_max_queue(:workspace_mutation),
    do: int_env("TREEDX_WORKSPACE_MUTATION_MAX_QUEUE", 1_000)

  def worker_pool_max_queue(:graph), do: int_env("TREEDX_GRAPH_MAX_QUEUE", 500)
  def worker_pool_max_queue(:snapshot), do: int_env("TREEDX_SNAPSHOT_MAX_QUEUE", 200)
  def worker_pool_max_queue(:import), do: int_env("TREEDX_IMPORT_MAX_QUEUE", 100)

  def worker_pool_queue_timeout(:repository_query),
    do: int_env("TREEDX_REPOSITORY_QUERY_QUEUE_TIMEOUT_MS", 30_000)

  def worker_pool_queue_timeout(:workspace_mutation),
    do: int_env("TREEDX_WORKSPACE_MUTATION_QUEUE_TIMEOUT_MS", 30_000)

  def worker_pool_queue_timeout(:graph), do: int_env("TREEDX_GRAPH_QUEUE_TIMEOUT_MS", 45_000)

  def worker_pool_queue_timeout(:snapshot),
    do: int_env("TREEDX_SNAPSHOT_QUEUE_TIMEOUT_MS", 60_000)

  def worker_pool_queue_timeout(:import), do: int_env("TREEDX_IMPORT_QUEUE_TIMEOUT_MS", 60_000)

  def execution_timeout_ms, do: int_env("TREEDX_HEAVY_OPERATION_EXECUTION_TIMEOUT_MS", 0)
