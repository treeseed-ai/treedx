defmodule TreeDx.Exec do
  @moduledoc false

  alias TreeDx.Exec.{Materializer, Policy, Runner}
  ...
  def run(workspace_id, params, principal) do
    ...
    outcome =
      with {:ok, ctx} <- context(workspace_id, principal, capability_for(mode)),
        ...
        TreeDx.Audit.append("exec.completed", %{
    ...
    case outcome do
      {:error, error} ->
        TreeDx.Audit.append("exec.rejected", %{
          actor_id: actor_id(principal),
          tenant_id: tenant_id(principal),
          workspace_id: workspace_id,
          operation: "workspace.exec",
  ...
  defp audit_started(ctx, workspace_id, mode, _command) do
    TreeDx.Audit.append("exec.started", %{
  ...
  defp persist_change(ctx, before, after_snapshot, path) do
    with :ok <- TreeDx.Files.PathPolicy.authorize(ctx.workspace, path, false) do
      cond do
