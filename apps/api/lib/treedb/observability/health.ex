defmodule TreeDb.Observability.Health do
  @moduledoc false
  ...
  def readiness(opts \\ []) do
    checks = [
      ...
      check("data_dir_exists", fn -> exists?(TreeDb.Store.data_dir()) end),
      check("storage_manifest", fn ->
        exists?(Path.join(TreeDb.Store.data_dir(), "catalog/manifest.tdb"))
      end),
      check("storage_lock", fn -> storage_lock_ok?() end),
      check("storage_replay", fn -> storage_replay_ok?(opts) end),
      check("native_loaded", fn ->
        if(Code.ensure_loaded?(TreeDb.Native), do: :ok, else: {:error, %{}})
      end)
    ]
  ...
  def deep(opts \\ []) do
    ...
    checks = [
      ...
      check("graph_store_readable", fn ->
        exists_or_empty?(Path.join(TreeDb.Store.data_dir(), "graph"))
      end),
      check("repository_placements_readable", fn ->
        exists_or_empty?(Path.join(TreeDb.Store.data_dir(), "federation"))
      end),
      check("audit_append_path", fn ->
        exists_or_empty?(Path.join(TreeDb.Store.data_dir(), "audit"))
      end),
      verifier_check()
    ]
  ...
  defp storage_lock_ok? do
    if TreeDb.AdminStorage.storage_mode() == "read_only_recovery" do
      :skipped
    else
      exists?(Path.join(TreeDb.Store.data_dir(), ".treedb.lock"))
    end
  end

  defp storage_replay_ok?(opts) do
    case Keyword.get(opts, :storage_check) || TreeDb.AdminStorage.check() do
  ...
  defp data_dir_writable? do
    path = Path.join(TreeDb.Store.data_dir(), ".health-write-check")
    File.write!(path, "ok")
    File.rm(path)
    :ok
  end
  ...
  defp verifier_check do
    if System.get_env("TREEDB_HEALTH_CHECK_AUTH_PROVIDER") == "true" do
      check("auth_provider", fn ->
        try do
          timeout_ms = env_int("TREEDB_HEALTH_CHECK_TIMEOUT_MS", 2_000)

          task =
            Task.async(fn ->
              if TreeDb.Auth.mode() == "connected" do
                case TreeDb.Auth.Connected.validate_config() do
                  :ok ->
                    :ok
