defmodule TreeDb.Observability.Telemetry do
  @moduledoc false
  use GenServer

  alias TreeDb.Observability.Metrics

  @handler_id "treedb-observability-telemetry"
  ...
  def handle_event([:phoenix, :endpoint, :stop], measurements, metadata, _config) do
    ...
    Metrics.incr("treedb_http_requests_total", labels)
    ...
    if status >= 400 do
      Metrics.incr("treedb_http_errors_total", Map.put(labels, :error_code, error_code(conn)))
    end
  ...
  defp error_code(conn) do
    conn.assigns[:treedb_error_code] || "unknown"
  rescue
    _ -> "unknown"
  end
end
