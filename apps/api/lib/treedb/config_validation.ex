defmodule TreeDb.ConfigValidation do
  @moduledoc false
  ...
  def validate_boot! do
    case validate_env(System.get_env()) do
      ...
      {:error, errors} ->
        ...
        raise "Invalid TreeDB production configuration: #{message}"
    end
  end
  ...
  defp production?(env), do: env["MIX_ENV"] == "prod" or env["TREEDB_ENV"] == "prod"
  ...
  defp reject_dev_auth(errors, env) do
    auth_mode = env["TREEDB_AUTH_MODE"] || "dev"
  ...
  defp validate_connected_auth(errors, env) do
    if production?(env) and env["TREEDB_AUTH_MODE"] == "connected" do
      verifier = env["TREEDB_AUTH_VERIFIER"]
      ...
      errors =
        if verifier == "hs256_dev" and env["TREEDB_ALLOW_DEV_VERIFIER_IN_PROD"] != "true" do
          add(
            errors,
      ...
      case verifier do
        "hs256_dev" ->
          require_keys(errors, env, [
            {"TREEDB_JWT_ISSUER", "missing_jwt_issuer"},
            {"TREEDB_JWT_AUDIENCE", "missing_jwt_audience"},
            {"TREEDB_JWT_HS256_SECRET", "missing_hs256_secret"}
          ])
        ...
        "jwks_oidc" ->
          require_keys(errors, env, [
            {"TREEDB_JWT_ISSUER", "missing_jwt_issuer"},
            {"TREEDB_JWT_AUDIENCE", "missing_jwt_audience"},
            {"TREEDB_JWKS_URL", "missing_jwks_url"},
            {"TREEDB_JWT_ALLOWED_ALGS", "missing_allowed_algs"}
          ])
  ...
  defp validate_exec_backend(errors, env) do
    backend = env["TREEDB_EXEC_BACKEND"] || "direct_dev"

    errors =
      if production?(env) and backend == "direct_dev" and
           env["TREEDB_ALLOW_DIRECT_EXEC_IN_PROD"] != "true" do
        add(
          errors,
    ...
    if production?(env) and backend in ["external_worker", "firecracker_or_microvm"] do
      errors
      |> require_keys(env, [{"TREEDB_EXEC_WORKER_URL", "missing_exec_worker_url"}])
      |> require_one(
        env,
        ["TREEDB_EXEC_WORKER_TOKEN", "TREEDB_EXEC_WORKER_HMAC_SECRET"],
  ...
  defp validate_git_transport(errors, env) do
    provider = env["TREEDB_REMOTE_CREDENTIAL_PROVIDER"] || "none"

    errors =
      if truthy?(env["TREEDB_GIT_EXTERNAL_TRANSPORT_ENABLED"]) and provider == "none" do
        add(
          errors,
          "missing_remote_credential_provider",
          "external Git transport requires a credential provider.",
          "TREEDB_REMOTE_CREDENTIAL_PROVIDER"
        )
      else
        errors
      end

    if truthy?(env["TREEDB_GIT_SSH_ENABLED"]) do
      errors
      |> require_keys(env, [{"TREEDB_GIT_SSH_KNOWN_HOSTS", "missing_ssh_known_hosts"}])
  ...
  defp validate_restore(errors, env) do
    if truthy?(env["TREEDB_STORAGE_RESTORE_ENABLED"]) and
         env["TREEDB_STORAGE_RESTORE_ACK"] != "true" do
      add(
        errors,
        "restore_ack_required",
        "storage restore requires explicit acknowledgement.",
        "TREEDB_STORAGE_RESTORE_ACK"
      )
    else
      errors
    end
  ...
  defp validate_data_dir(errors, env) do
    cond do
      ...
      blank?(env["TREEDB_DATA_DIR"]) ->
        add(errors, "missing_data_dir", "TREEDB_DATA_DIR is required.", "TREEDB_DATA_DIR")

      Path.type(env["TREEDB_DATA_DIR"]) != :absolute ->
        add(errors, "relative_data_dir", "TREEDB_DATA_DIR must be absolute.", "TREEDB_DATA_DIR")

      not writable_dir?(env["TREEDB_DATA_DIR"]) ->
        add(errors, "unwritable_data_dir", "TREEDB_DATA_DIR must be writable.", "TREEDB_DATA_DIR")
  ...
  defp validate_logger(errors, env) do
    if production?(env) do
      case Application.get_env(:logger, :console, [])[:format] do
        {TreeDb.Observability.JsonLogFormatter, :format} ->
          errors
  ...
  defp writable_dir?(path) do
    File.mkdir_p(path)
    probe = Path.join(path, ".treedb-config-write-check-#{System.unique_integer([:positive])}")
