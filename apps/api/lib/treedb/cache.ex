defmodule TreeDb.Cache do
  ...
  def get_or_load(table, key, ttl_ms, max_entries, max_bytes, loader) do
    ...
    case lookup(table, key, now, ttl_ms) do
      {:ok, value} ->
        Metrics.incr("treedb_cache_hits_total", %{cache: cache_name(table)})
        {:ok, value}

      :miss ->
        Metrics.incr("treedb_cache_misses_total", %{cache: cache_name(table)})
  ...
  defp publish_stats(table) do
    ...
    Metrics.put_gauge("treedb_cache_entries", stats.entries, labels)
    Metrics.put_gauge("treedb_cache_approx_bytes", stats.approx_bytes, labels)
  end
