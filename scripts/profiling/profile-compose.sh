...
usage() {
  cat <<'USAGE'
Usage:
  scripts/profiling/profile-compose.sh MODE [--dev-api] [--no-clean] [--no-build] [--config]
...
Examples:
  ...
  TREEDX_PROFILE_CONCURRENCY=200 scripts/profiling/profile-compose.sh read-heavy
  TREEDX_PROFILE_DURATION=2h scripts/profiling/profile-compose.sh soak
