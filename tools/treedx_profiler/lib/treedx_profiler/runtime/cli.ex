defmodule TreeDxProfiler.CLI do
  ...
  def main(argv) do
    case parse(argv) do
      ...
      {:error, message} ->
        ...
        IO.puts(:stderr, TreeDxProfiler.CLISupport.usage())
        System.halt(1)
    end
  end
    ...
    case OptionParser.parse(argv, strict: @switches, aliases: @aliases) do
      {opts, [], []} ->
        if Keyword.get(opts, :help, false) do
          {:help, TreeDxProfiler.CLISupport.usage()}
        else
          normalize(opts)
        end
  ...
  defp normalize(opts) do
    profile_id = Keyword.get(opts, :profile_id) || TreeDxProfiler.CLISupport.default_profile_id()
    ...
    normalized = %{
      ...
      reliability_budget:
        Keyword.get(
          opts,
          :reliability_budget,
          Path.join([TreeDxProfiler.CLISupport.profiler_root(), "reliability_budget.yaml"])
        ),
