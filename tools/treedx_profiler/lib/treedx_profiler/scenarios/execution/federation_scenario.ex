defmodule TreeDxProfiler.FederationScenario do
  @moduledoc false

  alias TreeDxProfiler.{FederationAssertions, FederationTopology}
  ...
  def setup_spillover_probe(state) do
    enabled? =
      System.get_env("TREEDX_PROFILE_FEDERATION_SPILLOVER_PROBE", "false") in ~w(true 1 yes on)
    ...
    case Req.post(node_a.url <> "/api/v1/repos/#{repo_id}/files/read",
      ...
      {:ok, %{status: status, headers: headers, body: body}} when status in 200..299 ->
        route =
          headers
          |> Enum.find_value(fn
            {"x-treedx-route-reason", value} ->
              header_value(value)

            {key, value} when is_binary(key) ->
              if String.downcase(key) == "x-treedx-route-reason", do: header_value(value)
